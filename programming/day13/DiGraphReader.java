...
public class DiGraphReader implements IGraphReader {
    ...
    public static void main(String[] argv) throws Exception {
        // Switch between the Breadth First and depth First instatiator lines
        BreadthFirstSearcher s = new BreadthFirstSearcher();
        ...
        hello = s.pathExists(g, start, end);
        System.out.println(hello);

        INode<String>[] nodes = g.getNodeSet();
        System.out.println("number of nodes: " + nodes.length);

        IList<INode<String>> fin = new ArrayList<INode<String>>();
        fin = s.getPath(g, start, end);
        for(int j = 0; j < fin.size(); j++){
          System.out.print(" " + fin.fetch(j).getValue() + " ");
        }
    }
  }
